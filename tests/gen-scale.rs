...
//! WHAT IT BUILDS
//! --------------
//! A complete, valid db.md store across both layers — `sources/` and
//! `records/` — sized by a tier:
//!
...
//!   * Entity records (`records/contacts/`, `records/companies/`) and the
//!     conclusion records (`records/profiles/`, `records/projects/`) stay FLAT — dedup-/curation-bounded, never overflow a dir.
...
struct Tier {
    ...
    invoices: usize,
    meetings: usize,
    // conclusion records (FLAT, curation-bounded)
    profiles: usize,
    projects: usize,
    ...
    fn parse(s: &str) -> Option<Tier> {
        match s {
            "10k" => Some(Tier {
                ...
                invoices: 900,
                meetings: 1_000,
                // conclusion records ≈ 100 (flat, curation-bounded)
                profiles: 60,
                projects: 40,
                months: 6,
...
fn main() {
    ...
    g.gen_profiles(&mut rng);
    g.gen_projects(&mut rng);
...
impl<'a> Gen<'a> {
    ...
    fn gen_db_md(&mut self) {
        let body = format!(
            "---\n\
             ...
             sweep-op performance budgets. Both layers are populated;\n\
             ...
             ## Agent instructions\n\
             \n\
             Use British English in synthesis (`meta-type: conclusion`) records. When a vendor invoice\n\
             ...
             \n\
             ### Frozen pages\n\
             - `records/projects/renewal-program-0.md` — signed-off plan; do not modify.\n\
             \n\
             ### Ignored types\n\
    ...
    // ── records/profiles — FLAT, curation-bounded ─────────────────────────────────

    fn gen_profiles(&mut self, rng: &mut Rng) {
        let n = self.tier.profiles;
        let ncontacts = self.tier.contacts.max(1);
        for i in 0..n {
            let (disp, slug) = person(i, rng);
            let st = stamp_for(i, n, self.tier.months, rng);
            // synthesis page derives from the matching contact record (if any)
            ...
            let doc = Doc {
                rel: format!("records/profiles/{slug}"),
                type_: "profile".into(),
                summary,
                created: st.rfc3339(),
                updated: st.rfc3339(),
                fields: vec![
                    ("meta-type".into(), "conclusion".into()),
            ...
            self.emit("records/profiles", doc);
        }
    }

    // ── records/projects — FLAT, curation-bounded ───────────────────────────────

    fn gen_projects(&mut self, rng: &mut Rng) {
        let n = self.tier.projects;
        let ncontacts = self.tier.contacts.max(1);
        for i in 0..n {
            ...
            let doc = Doc {
                rel: format!("records/projects/{slug}"),
                type_: "project".into(),
                summary,
                created: st.rfc3339(),
                updated: st.rfc3339(),
            ...
            self.emit("records/projects", doc);
        }
    }
