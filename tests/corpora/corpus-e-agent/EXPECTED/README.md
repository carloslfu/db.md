...
each `summary`, each wiki body, each full-path wiki-link — was **authored
...
- the **`newsletter` Ignored-types policy** + the `transient` tag → the
  Design Weekly digest produces NO record and NO wiki page (it remains a
...
- the **schema `contact.company (required, link to records/companies/)`** +
  ...
  NOTES "resolution (a)");
- the **British-English-in-wiki rule** → "organise" / "synthesise" in the
  wiki bodies, verbatim source values in `records/`;
- the **$45k SOW fee is a receivable, not an expense** → it is narrated in
  the project wiki page but never modelled as an `expense`.
...
- a record exists for **every** entity `NOTES.md` requires (3 counterparty
  ...
  newsletter);
- the wiki pages exist and their bodies link to their evidence (records +
...
- **Golden output (this tree):** everything the curator session produces —
  `records/**`, `wiki/**`, the full `index.md` hierarchy (root + every layer
...
write-through the loop uses for `records/`/`wiki/`. The resulting
