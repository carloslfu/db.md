...
fn query_limit_is_path_sorted_with_loose_and_type_folders() {
    ...
    // a path sort before `--limit`, `fm query --limit 2` dropped the true #1
    // (`records/aaa.md`) and kept the wrong subset. `fm query` must apply the same
    // path sort `dbmd query` does, so the top-2 is the path-ascending top-2.
    let tmp = tempfile::TempDir::new().expect("tempdir");
    let store = tmp.path();
    write_db_md(store);
    write_file(
    ...
    assert_eq!(
        ...
        "fm query --limit 2 must return the path-ascending top-2 (loose file first)"
    );
    ...
    let canonical = dbmd()
        ...
        .assert()
        .success();
    let canonical_got: Vec<String> = String::from_utf8(canonical.get_output().stdout.clone())
        .unwrap()
        .lines()
        .map(str::to_string)
        .collect();
    assert_eq!(
        got, canonical_got,
        "fm query and the canonical query must return the same top-2"
    );
}
