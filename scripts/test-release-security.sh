...
require_fixed 'release_id: ${{ steps.draft.outputs.release_id }}' "$workflow"
require_fixed 'repos/${GITHUB_REPOSITORY}/releases?per_page=100' "$workflow"
require_fixed 'multiple releases claim tag $tag' "$workflow"
require_fixed 'repos/${GITHUB_REPOSITORY}/releases/${RELEASE_ID}' "$workflow"
require_fixed 'RELEASE_ID: ${{ needs.publish.outputs.release_id }}' "$workflow"
reject_fixed 'repos/${GITHUB_REPOSITORY}/releases/tags/${tag}" --jq .id' "$workflow"
...
require_fixed 'rustup docker tar xcode-select' "$controller"
require_fixed 'docker info >/dev/null 2>&1' "$controller"
require_fixed 'rustup toolchain install "$LINUX_RUST_TOOLCHAIN"' "$controller"
require_fixed '--profile minimal --force-non-host' "$controller"
...
require_fixed ')" = amd64 || die "Linux builder image is not linux/amd64: $builder_image"' "$controller"
require_fixed 'docker run --rm --platform linux/amd64' "$controller"
require_fixed '--volume "$release_source:/project:ro"' "$controller"
require_fixed '--volume "$cargo_home:/cargo"' "$controller"
require_fixed '--volume "$linux_rust_sysroot:/rust:ro"' "$controller"
require_fixed '--volume "$target_dir:/target"' "$controller"
...
require_fixed 'needs: [version, publish, publish-crates]' "$workflow"
